...
<body>
          ...
          <div class="mb-4 d-flex align-items-center">
            ...
            <div class="modal fade-modal" id="update" tabindex="-1" aria-labelledby="nuevoProyecto" aria-hidden="true">
              <div class="modal-dialog-modal" style="min-width: 75%;">
                <div class="modal-content-modal">
                  <div class="modal-header-modal">
                    <h2 class="modal-title-modal" id="nuevoProyecto">Datos</h2>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span>&times;</span>
                    </button>
                  </div>

                  <div class="modal-body-modal" style='display-none'>
                    <form method="post" action="jugadores2.php" enctype="multipart/form-data">
                      ...
                      <div class="form-group">
                        ...
                        <input type="number" step="0.01" class="form-control" id="estatura1" name="estatura1"
                          placeholder="Estatura" required readonly>
                      </div>
                      <div class="form-group">
                      ...
                      <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="nacionalidad1" name="nacionalidad1"
                          placeholder="Nacionalidad" required readonly>
                      </div>
                      <div class="form-group">
                        <label for="equipo1">Equipo</label>
                        <input type="text" class="form-control" id="equipo1" name="equipo1"
                          value="<?php echo $nombreEquipo; ?>" placeholder="Equipo">
                      </div>
                      <button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
                      <button type="submit" name="accion" value="<?php echo $accion; ?>" class="btn btn-primary">Cambiar
                        Equipo</button>
  ...
  function realizarRedireccion() {
      ...
      window.location.href = 'jugadores2.php';
  }
  ...
  function obtenerJugadores()
  {
    ...
    try {
      ...
      $stmt = $pdo->prepare("SELECT j.*
              FROM Entrenador e
              INNER JOIN Equipo eq ON e.ID = eq.IdEntrenador
              INNER JOIN jugadores j ON eq.Nombre = j.equipo
              WHERE e.ID = :sesion_id");
      $stmt->bindParam(':sesion_id', $sesion_id, PDO::PARAM_STR);
      $stmt->execute();

      // Utiliza fetchAll para obtener todos los resultados como un array asociativo
  ...
  // LLENAR CON EQUIPO DEL ENTRENADOR
      ...
      if ($row) {
        ...
        echo '<script>
                    document.addEventListener("DOMContentLoaded", function() {
                        var equipo1Input = document.getElementById("equipo1");
                        if (equipo1Input) {
                            equipo1Input.value = "' . $nombreEquipo . '";
                        }
                    });
                </script>';
      }
